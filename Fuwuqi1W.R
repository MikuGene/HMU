...
HNSC <- scRNA_3(HNSC_sc,Reso = 0.6,pmax = 15,name = "HNSC_10k")
## 200 Inf 0 40 ; 17 ##
...
VlnPlot(HNSC,features =  c("KRT19","CD3D","COL3A1","CLU","MS4A1","AQP1","APOC1","MKI67","CDKN2A"),pt.size = 0.4,same.y.lims = T)
FeaturePlot(HNSC,c("KRT19","CD3D","COL3A1","CLU","MS4A1","AQP1","APOC1","MKI67","CDKN2A"),coord.fixed = T)
...
HNSC_can <- HNSC@assays$RNA@counts[,colnames(HNSC@assays$RNA@counts) %in% names(HNSC@active.ident)[HNSC@active.ident == "Cancer cells"]]
HNSC2 <- scRNA_3(HNSC_can,Reso = 0.4,pmax = 15,name = "HNSC_Can")
## 200 Inf 0 40 ; 13 ##
...
plot_cell_trajectory(HNSC2_Ps,color_by = HNSC2_Ps)
## done. ##


## WGCNA ##
HNSC2 <- readRDS("HNSC_Can.rds")
HNSC2_M <- as.matrix(HNSC2@assays$RNA@data)
HNSC2_M <- remRow(HNSC2_M,Rem = 0.9)
rm(HNSC2)
HNSC2_Ps <- readRDS("HNSC2_Ps.rds")
HNSC2_Cli <- data.frame(Cells = colnames(HNSC2_M), Pseudotime = HNSC2_Ps$Pseudotime)
rm(HNSC2_Ps)
HNSC2_WG_info <- WGCNA_CliCustom(HNSC2_Cli,HNSC2_M,IDname = "Cells")
HNSC2_WG <- WGCNA_TOMmap(HNSC2_WG_info,Cutsample = F,Map = F)
saveRDS(HNSC2_WG_info,paste0("HNSC2_WG_info",".rds"))
saveRDS(HNSC2_WG,paste0("HNSC2_WG",".rds"))
## done. ##
## Plot. ##
HNSC2_WG_info <- readRDS("HNSC2_WG_info.rds")
HNSC2_WG <- readRDS("HNSC2_WG.rds")
HNSC2_WG_link <- WGCNA_CliLink(HNSC2_WG_info,HNSC2_WG)
## Pseu ##
HNSC2_WG_blue <- WGCNA_Detail(HNSC2_WG_link,HNSC2_WG,Color = "blue",name = "HNSC2_blue",Trait = "Pesudotime",Cys = T,Cliorder = 1)
HNSC2_WG_blue_Rn <- Enrich(HNSC2_WG_blue[[1]],dir = "HNSC2_blue",IDname = "HNSC2_blue")
## done. ##
